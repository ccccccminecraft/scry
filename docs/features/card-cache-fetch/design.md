...
| 通信方式 | 同期 POST（完了まで待機）+ フロントエンド側スピナー |
...
4. 取得件数・失敗件数・失敗カード名リストを返す

`scryfall_enabled=false` の場合は処理を行わず `403` を返す。タイムアウトは uvicorn のデフォルト 300 秒に依存（1000件で約50秒の想定）。
...
- `fetchMissingCardData(filters)`: `POST .../fetch-missing` を呼び出し、取得件数・失敗件数・失敗カード名リストを返す。axios の timeout を 300 秒に明示する（長時間リクエスト対策）
...
| ネットワークエラー | エラートースト表示。途中まで取得済みのカードは保存される。エラー分は miss に記録しない |
...
| `frontend/src/api/matches.ts` | `CardDictionaryCount` 型変更・`fetchMissingCardData()`・`resetCardCacheMiss()` 追加 |
| `frontend/src/api/settings.ts` | `fetchCardCacheMissCount()`・`deleteAllCardCacheMiss()` 追加 |
| `frontend/src/views/AIExportView.vue` | カード辞書タブに取得ボタン・リセットボタン・miss 件数表示を追加 |
