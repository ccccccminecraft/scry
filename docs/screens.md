...
**共通状態遷移**: idle → importing（プログレスバー＋中断ボタン） → batch_result（件数表示）

インポート中は「中断する」ボタンを表示。押下で `POST /api/import/cancel` を送信し、処理を中断する。中断後は batch_result 画面に遷移し「中断スキップ」件数を表示。

---

### MatchListView（`/matches`）

- `GET /api/matches` で一覧取得（10件/ページ）
- フィルター: プレイヤー・相手・使用デッキ（複数選択可）・バージョン・相手デッキ（複数選択可）・フォーマット・日付範囲
- フィルターはボタン＋モーダル形式（`FilterBar` コンポーネント）
...
- フィルター: プレイヤー（必須）・相手・使用デッキ（複数選択可）・バージョン・相手デッキ（複数選択可）・フォーマット・日付範囲
- フィルターはボタン＋モーダル形式（`FilterBar` コンポーネント）
- `GET /api/stats` で集計統計（勝率・マリガン率・ターン数・先手後手別等）
- `GET /api/stats/cards` で相手カード出現統計（折りたたみ表示）
- グラフ: vue-echarts（勝率推移）
- 対戦相手デッキ別勝率テーブル（Top 10、対戦数/勝率でソート可）
- サマリー行左端に使用デッキのデッキタイル（代表カード画像）を表示（単一デッキ選択時）
...
- フィルター指定で対戦データを Markdown 形式でエクスポート（`.md` ファイルダウンロード）
- `GET /api/matches/export/count` で対象件数プレビュー
- 出力内容を6つのチェックボックスで独立制御:
  - サマリー / デッキ別勝率 / カード統計 / デッキリスト / 対戦一覧 / アクション詳細
  - アクション詳細は「対戦一覧」がオンのときのみ有効
- 件数上限: 「制限なし（全件）」または直近N件
- チェックボックスの状態は localStorage に保存（ページ遷移後も維持）
- 大量エクスポート時は確認ダイアログを表示
...
- Scryfall API を利用する（チェックボックス）: オフ時はフォーマット推定・カード画像取得をスキップ
- デフォルト期間フィルター: 全期間 / 今月 / 直近30日 / カスタム開始日から選択
