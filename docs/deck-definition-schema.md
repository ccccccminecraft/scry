...
Scry のアーキタイプ定義インポート/エクスポートに使用する JSON フォーマット仕様。
...
| `definitions` | Definition[] | ✓ | アーキタイプ定義の配列 |
...
8. 同名のアーキタイプ定義が既に存在する場合は**スキップ or 上書き**をユーザーが選択できる
...
|---|---|
| 1.0 | 初版 |
| 1.0 (追記) | `exclude_cards` フィールドを追加。除外カードが使用されていた場合にアーキタイプ定義をマッチさせない機能 |
